...
open class GenericDecoder {
    public static let shared = GenericDecoder()
    ...
    /// Decodes a top-level value of the given type from the given generic value.
    ///
    ...
    open func decode<T : Decodable>(_ type: T.Type, from genericValue: Any) throws -> T {
        let decoder = __JSONDecoder(referencing: genericValue, options: self.options)
        guard let value = try decoder.unbox(genericValue, as: type) else {
...
fileprivate class __JSONDecoder : Decoder {
    ...
    fileprivate let options: GenericDecoder._Options
    ...
    fileprivate init(referencing container: Any, at codingPath: [CodingKey] = [], options: GenericDecoder._Options) {
    ...
    fileprivate init(referencing decoder: __JSONDecoder, wrapping container: [String : Any]) {
        ...
        case .convertFromSnakeCase:
            ...
            self.container = Dictionary(container.map {
                key, value in (GenericDecoder.KeyDecodingStrategy._convertFromSnakeCase(key), value)
